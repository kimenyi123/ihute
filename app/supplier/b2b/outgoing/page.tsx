...
import { listOutgoing, B2BOrder, B2BApiError } from "@/lib/b2bApi";
...
export default function B2BOutgoingPage() {
  ...
  useEffect(() => {
    ...
    console.log("Outgoing page auth check:", { isAuthenticated, user, userRole: user?.role });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">Loading outgoing orders...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white border-b shadow-sm mb-6">
        <div className="container mx-auto px-6 py-4">
          ...
          <h1 className="text-3xl font-bold text-slate-900">
            Outgoing Orders / Byinshi wahabitswe
          </h1>
          <p className="text-slate-600 mt-2">
            Orders you've placed to other sellers
          </p>
        </div>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <option value="B2B_DRAFT">Draft</option>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span>Your Orders ({total} total)</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {orders.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-600 text-lg">No outgoing orders found</p>
                <p className="text-slate-500 text-sm mt-2">
                  {statusFilter ? "Try a different status filter" : "Start by uploading an Excel file or using Quick Buy"}
                </p>
              </div>
            ) : (
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b">
                      ...
                      <th className="text-left p-3 font-semibold text-slate-900">Child Orders</th>
                  ...
                  <tbody>
                    {orders.map((order) => (
                      <tr key={order.id} className="border-b hover:bg-slate-50">
                        ...
                        <td className="p-3">
                          <span className="font-medium">{order.childCount || 0}</span>
                        </td>
                        <td className="p-3">
