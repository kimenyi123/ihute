                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="lg:col-span-2 bg-white rounded-lg shadow p-4">
                                ...
                                >
                                    ...
                                    {/* Center Marker */}
                                    <Marker position={[data.center_lat, data.center_lng]}>
                                        <Popup>Your Operation Center</Popup>
                                    </Marker>

                                    {/* Current Coverage Circle */}
                                    <Circle
                                        center={[data.center_lat, data.center_lng]}
                                        radius={data.coverage_radius_km * 1000}
                                        pathOptions={{
                                            fillColor: '#10b981',
                                            fillOpacity: 0.15,
                                            color: '#10b981',
                                    ...
                                    {/* Suggested Coverage Circle */}
                                    <Circle
                                        center={[data.center_lat, data.center_lng]}
                                        radius={data.suggested_radius_km * 1000}
                                        pathOptions={{
                                            fillColor: '#f59e0b',
                                            fillOpacity: 0.1,
                                            color: '#f59e0b',
                                    ...
                                    {/* Expansion Opportunity Markers */}
                                    {data.expansion_areas.map((area, index) => (
                                        <Circle
                                            key={index}
                                        ...
                                        {data.expansion_areas.length > 0 && (
                                            <div className="bg-blue-50 border-l-4 border-blue-500 p-3">
                                                ...
                                                <div className="text-blue-700 mt-1">
                                                    {data.expansion_areas.length} area(s) with high potential
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                        ...
                                        <div>
                                            <div className="flex justify-between text-sm mb-1">
                                                ...
                                                <span className="font-medium">
                                                    {Math.round((data.coverage_radius_km / data.suggested_radius_km) * 100)}%
                                                </span>
                                            </div>
                                            <div className="w-full bg-gray-200 rounded-full h-2">
                                                <div
                                                    className="bg-green-500 h-2 rounded-full"
                                                    style={{ width: `${Math.min(100, (data.coverage_radius_km / data.suggested_radius_km) * 100)}%` }}
                                                ></div>
                                            </div>
                                        </div>
                                    </div>
