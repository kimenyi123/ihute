...
import { isValidMmPinDigits, mmPinLength } from "@/lib/mobile-money-pin"

type Provider = "mtn" | "airtel"
type Step = "pick" | "details" | "otp" | "pin"
...
export function MobileMoneyPaymentWizard() {
  ...
  const payStart = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/payment/mobile-money/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider, phone: norm, amount: Math.round(amountNum) }),
      })
      const data = (await res.json()) as { ok?: boolean; sessionToken?: string; maskedPhone?: string; error?: string }
  ...
  const submitOtp = async () => {
    ...
    setError("")
    try {
      const res = await fetch("/api/payment/mobile-money/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionToken, otp: o }),
      })
      const data = (await res.json()) as { ok?: boolean; sessionToken?: string; error?: string }
      if (!res.ok || !data.ok || !data.sessionToken) {
        throw new Error(data.error || `OTP step failed (${res.status})`)
      }
      setSessionToken(data.sessionToken)
      setPin("")
      setStep("pin")
  ...
  const submitPin = async () => {
    if (!provider) return
    const p = digitsOnly(pin)
    if (!isValidMmPinDigits(provider, p)) {
      const n = mmPinLength(provider)
      setError(
        provider === "mtn"
          ? `Enter your ${n}-digit MTN MoMo PIN.`
          : `Enter your ${n}-digit Airtel Money PIN.`,
      )
      return
    }
    setLoading(true)
    setError("")
    try {
      const res = await fetch("/api/payment/mobile-money/complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionToken, pin: p }),
      })
      ...
      if (!res.ok || !data.ok || !data.transactionId) {
        throw new Error(data.error || `Payment failed (${res.status})`)
      }
      const q = new URLSearchParams({
        transactionId: data.transactionId,
        status: data.status ?? "SUCCESS",
        method: data.paymentMethod ?? (provider === "mtn" ? "MTN Mobile Money" : "Airtel Money"),
        phone: data.phoneMasked ?? "",
        amount: String(data.amountRwf ?? Math.round(amountNum)),
      })
      router.push(`/payment/track?${q.toString()}`)
    } catch (e: unknown) {
  ...
  return (
    <div className="rounded-2xl border border-emerald-100 bg-gradient-to-b from-emerald-50/60 to-white p-6 shadow-lg shadow-emerald-900/5">
      <div className="mb-5 flex flex-col gap-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold tracking-tight text-emerald-950">Mobile money</h2>
        </div>
        <p className="text-xs text-emerald-900/75">MTN or Airtel · demo (no real charges)</p>
      </div>
      ...
      {step === "pick" ? (
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          >
            ...
            <span className="font-semibold text-emerald-950">MTN Mobile Money</span>
            <span className="text-[11px] text-emerald-900/70">Phone + amount</span>
          </button>
          <button
            type="button"
            onClick={() => selectProvider("airtel")}
            className={cn(
              "flex min-h-[88px] flex-col items-start gap-1 rounded-xl border-2 p-4 text-left transition-all duration-200 hover:scale-[1.01] hover:shadow-md",
              "border-red-300/50 bg-red-50/60",
            )}
          >
            <span className="text-xl" aria-hidden>
              📲
            </span>
            <span className="font-semibold text-emerald-950">Airtel Money</span>
            <span className="text-[11px] text-emerald-900/70">Phone + amount</span>
          </button>
        </div>
      ) : null}
      ...
      {step === "details" && provider ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-semibold capitalize text-emerald-950">{provider}</p>
            <Button type="button" variant="ghost" size="sm" className="shrink-0 text-emerald-800" onClick={resetFlow}>
              Change
            </Button>
          </div>
          <div>
            <Label htmlFor="mm-phone" className="text-emerald-900">
              MoMo wallet number
            </Label>
            <Input
              id="mm-phone"
              ...
              placeholder={provider === "mtn" ? "078 …" : "073 …"}
              className={cn(
                "mt-1.5 min-h-[48px] border-emerald-200 text-base transition-shadow focus-visible:border-emerald-500 focus-visible:ring-emerald-500/20",
                phoneValid && "ring-2 ring-emerald-400/30",
              )}
            />
          </div>
          <div>
            <Label htmlFor="mm-amount" className="text-emerald-900">
              Amount (RWF)
            </Label>
            <Input
              id="mm-amount"
              ...
              onChange={(e) => setAmountStr(e.target.value.replace(/[^\d.,]/g, ""))}
              placeholder="5,000"
              className="mt-1.5 min-h-[48px] border-emerald-200 text-base focus-visible:border-emerald-500 focus-visible:ring-emerald-500/20"
            />
          </div>
          {phoneValid ? (
            <div className="animate-in fade-in rounded-xl border border-emerald-200 bg-emerald-50/50 px-3 py-2 text-xs text-emerald-900">
              <Lock className="mb-1 inline h-3.5 w-3.5 align-text-bottom text-emerald-700" aria-hidden />
              Next: OTP (6 digits), then PIN (MTN: 5 digits · Airtel: 4) — auto-continues when each step is complete.
            </div>
          ) : null}
          <Button
            type="button"
            className="min-h-[48px] w-full bg-gradient-to-r from-emerald-600 to-emerald-700 font-bold text-white shadow-md shadow-emerald-700/20 hover:from-emerald-500 hover:to-emerald-600"
            disabled={loading}
            onClick={() => void payStart()}
          >
            {loading ? (
      ...
      {step === "otp" ? (
        <div className="space-y-4">
          <p className="text-xs text-emerald-900/80">
            Code to <span className="font-mono font-semibold text-emerald-950">{maskedPhone || "your phone"}</span> ·
            any 6 digits (demo)
          </p>
          <div>
            <Label htmlFor="mm-otp" className="text-emerald-900">
              OTP
            </Label>
            <Input
              id="mm-otp"
              ...
              value={otp}
              onChange={(e) => {
                const next = digitsOnly(e.target.value).slice(0, 6)
                const prevLen = digitsOnly(otp).length
                setOtp(next)
                if (next.length === 6 && prevLen < 6) {
                  queueMicrotask(() => void submitOtp())
                }
              }}
              maxLength={6}
              className="mt-1.5 min-h-[48px] border-emerald-200 font-mono text-lg tracking-widest focus-visible:border-emerald-500 focus-visible:ring-emerald-500/20"
              placeholder="••••••"
            />
          </div>
          {loading ? (
          ...
          <div className="flex gap-2">
            <Button
              type="button"
              variant="outline"
              className="min-h-[44px] flex-1 border-emerald-200 text-emerald-900"
              disabled={loading}
            ...
            <Button
              type="button"
              className="min-h-[44px] flex-1 bg-emerald-600 font-semibold hover:bg-emerald-500"
              disabled={loading}
      ...
      {step === "pin" && provider ? (
        <div className="space-y-4">
          <p className="text-xs text-emerald-900/80">
            {provider === "mtn"
              ? "MTN MoMo: 5-digit wallet PIN (demo — test digits only)"
              : "Airtel Money: 4-digit wallet PIN (demo — test digits only)"}
          </p>
          <div>
            <Label htmlFor="mm-pin" className="text-emerald-900">
              PIN ({mmPinLength(provider)} digits)
            </Label>
            <Input
              id="mm-pin"
              type="password"
              ...
              value={pin}
              onChange={(e) => {
                const need = mmPinLength(provider)
                const next = digitsOnly(e.target.value).slice(0, need)
                const prevLen = digitsOnly(pin).length
                setPin(next)
                if (next.length === need && prevLen < need) {
                  queueMicrotask(() => void submitPin())
                }
              }}
              maxLength={mmPinLength(provider)}
              className="mt-1.5 min-h-[48px] border-emerald-200 font-mono text-lg tracking-widest focus-visible:border-emerald-500 focus-visible:ring-emerald-500/20"
              placeholder={provider === "mtn" ? "•••••" : "••••"}
            />
          </div>
          {loading ? (
            <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-900">
