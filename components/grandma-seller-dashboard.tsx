...
export type GrandmaSellerDashboardProps = {
  ...
  openOrders: OpenStat
  openCap?: number
  /** Static KPIs until APIs exist (S2) */
  clientsCount?: number
  clientStars?: number
  itemsCount?: number
  itemsValueRwf?: number
  salesCount?: number
  salesTotalRwf?: number
  onOrders: () => void
  onClients: () => void
  onItems: () => void
  onSales: () => void
...
export function GrandmaSellerDashboard({
  ...
  clientStars = 4,
  itemsCount = 25,
  itemsValueRwf = 53_000,
  salesCount = 124,
  salesTotalRwf = 1_000_000,
  onOrders,
  onClients,
  onItems,
  onSales,
  ...
  const stars = "★".repeat(Math.min(5, Math.max(0, clientStars))) + "☆".repeat(Math.max(0, 5 - clientStars))
    ...
    <section className="seller-screen space-y-3 px-3 pb-6 pt-2">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <span className="mt-3 text-2xl font-black text-[#17324d]">{clientsCount}</span>
        ...
        >
          ...
          <span className="mt-3 text-2xl font-black text-[#17324d]">{itemsCount}</span>
          <span className="mt-1 text-sm font-bold text-[#127fc0]">{formatRwf(itemsValueRwf)}</span>
        </button>
        ...
        >
          ...
          <span className="mt-3 text-2xl font-black text-[#17324d]">{salesCount}</span>
          <span className="mt-1 text-sm font-bold text-[#127fc0]">{formatRwf(salesTotalRwf)}</span>
        </button>
      </div>

      <p className="px-1 text-center text-[11px] font-semibold text-[#6f8399]">
        Tap <span className="text-[#127fc0]">Orders</span> for live queue · other tiles coming soon
      </p>
    </section>
  )
}
